...
public class MainActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        if(savedInstanceState != null) {
            mygame.yourTurn = savedInstanceState.getBoolean("yourTurn");
            mygame.setCurrentPhase(savedInstanceState.getInt("currentPhase"));
            mygame.yourTurnTriggers = savedInstanceState.getIntArray("yourTurnTriggers");
            mygame.enemyTurnTriggers = savedInstanceState.getIntArray("enemyTurnTriggers");
            turnPreview.setChecked(savedInstanceState.getBoolean("turnPreview"));
            updateVisuals();
            changeHighlight();
        }
        changeHighlight();
    ...
    protected void onSaveInstanceState(Bundle outState){
        ...
        outState.putIntArray("yourTurnTriggers",mygame.yourTurnTriggers);
        outState.putIntArray("enemyTurnTriggers",mygame.enemyTurnTriggers);
        outState.putBoolean("yourTurn",mygame.yourTurn);
        outState.putInt("currentPhase",mygame.getCurrentPhase());
        outState.putBoolean("turnPreview",turnPreview.isChecked());
    }
    public void nextPhase(){
        if(mygame.nextPhase()){
            ...
            view.setBackgroundColor(getResources().getColor(R.color.secondaryDarkColor));
            toast.show();
        }
        if(mygame.getCurrentPhase()==0 && turnPreview.isChecked()!= mygame.yourTurn){
            turnPreview.setChecked(!turnPreview.isChecked()) ;
        }
        changeHighlight();
        updateVisuals();
    }
    ...
    private void changeHighlight(){
        if(turnPreview.isChecked()) {
            endStepVisuals.setTextColor(getResources().getColor(R.color.yourturn));
            untapVisuals.setTextColor(getResources().getColor(R.color.yourturn));
            upkeepVisuals.setTextColor(getResources().getColor(R.color.yourturn));
            drawVisuals.setTextColor(getResources().getColor(R.color.yourturn));
            mainPhase1Visuals.setTextColor(getResources().getColor(R.color.yourturn));
            combatVisuals.setTextColor(getResources().getColor(R.color.yourturn));
            mainPhase2Visuals.setTextColor(getResources().getColor(R.color.yourturn));
            ...
            endStepVisuals.setTextColor(getResources().getColor(R.color.enemyturn));
            untapVisuals.setTextColor(getResources().getColor(R.color.enemyturn));
            upkeepVisuals.setTextColor(getResources().getColor(R.color.enemyturn));
            drawVisuals.setTextColor(getResources().getColor(R.color.enemyturn));
            mainPhase1Visuals.setTextColor(getResources().getColor(R.color.enemyturn));
            combatVisuals.setTextColor(getResources().getColor(R.color.enemyturn));
            mainPhase2Visuals.setTextColor(getResources().getColor(R.color.enemyturn));
            if (mygame.yourTurn == turnPreview.isChecked()) {
                switch (mygame.getCurrentPhase()) {
                    case 0:
                        untapVisuals.setTextColor(getResources().getColor(R.color.highlight));
...
//        view.getId();
    ...
    //from android documents
    public void openInfoPage() {
        Intent intent = new Intent((Intent.ACTION_VIEW));
        intent.setData(Uri.parse("https://primagames.com/tips/magic-gathering-phases-guide"));
